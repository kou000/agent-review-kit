...
- **リポジトリファイルビューア**: サイドバー下部の「リポジトリのファイル」（デフォルト折りたたみ）からリポジトリのファイル（追跡ファイル＋未追跡ファイル。`.gitignore` 対象は除外）をツリーで辿り、差分に含まれないファイルも読み取り専用パネル（ピンスタック）として差分の横に並べて参照できる。パネルからは `/file/<path>` の単独ページも開ける。配信はこの一覧に載っているファイルのみ（gitignore 済みの `.env` 等は出ない。差分生成と同じ境界）。ファイル内検索（`git grep`）だけは git の挙動どおり追跡ファイルのみが対象。
...
| GET | `/file/:path` | リポジトリファイル1件の読み取り専用ページ |
