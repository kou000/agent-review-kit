...
- **リポジトリファイルビューア**: サイドバー下部の「リポジトリのファイル」（デフォルト折りたたみ）から git 追跡ファイルをツリーで辿り、差分に含まれないファイルも読み取り専用パネル（ピンスタック）として差分の横に並べて参照できる。パネルからは `/file/<path>` の単独ページも開ける。配信は `git ls-files` に載っているファイルのみ（未追跡の `.env` 等は出ない）。
...
| GET | `/api/repo-files` | git 追跡ファイルの一覧（サイドバーのリポジトリツリー用） |
| GET | `/api/file?path=...` | 追跡ファイル1件の内容（Shiki ハイライト付き、1MB上限・バイナリ判定あり） |
| GET | `/file/:path` | 追跡ファイル1件の読み取り専用ページ |
