...
- **GitHub ライクな UI**: side-by-side 差分、Shiki による SSR シンタックスハイライト、行番号クリックで単一行コメント、Shift+クリックまたはドラッグで行範囲コメント、インラインのコメントスレッド表示（折りたたみ対応。コメントフォームとスレッドは**コメントした側の半分**に出る — 変更後（右）の行なら右側、変更前（左）の行なら左側）、未解決数バッジ、レビュー対象コミット一覧（Commits タブ相当）、ファイルごとの **Viewed（確認済み）チェック**（サーバー側に永続化され、そのファイルの差分が変わると自動で未確認に戻る）。
- **ファイル全体へのコメント**: 差分の帯（ファイル名の行）の「💬」から、行に紐づかないコメントをそのファイル**全体**に付けられる。「このファイルは分割したい」「そもそもなぜこのファイルを触っているのか」のように、どの1行にも属さない指摘・質問の置き場所。投稿したスレッドは帯の直下（差分表の上）に並び、サイドバーには `パス 全体` と出る。行コメント（`ファイル:L42`）とレビュー全体コメント（ファイルにも紐づかない）の中間の粒度。
...
- **ワンクリックの修正依頼**: 未解決のコメントカードにはどれでも「🔧 修正を依頼」ボタンが出る（AI の指摘だけでなく通常コメントにも、スレッドの先頭だけでなく返信カードにも）。押した場所がスレッドのどこであっても、返信はそのスレッドに付く。押すと「上記の通り修正してください」という `intent: "fix"` の返信が自分名義で投稿され、通常の返信と同じ経路でエージェントに届く。エージェントの回答を読んで「ではその通り直して」と頼むときに、返信を打ち込まずに済む。
- **手動修正**: 差分の行範囲やHTMLレビューの要素を選択して出るフォームの「✏️ 手動修正」ボタンで、AI を介さずその場で書き換えて直接適用できる。差分では working tree のファイルの該当行を書き換えて差分を自動再生成、HTMLレビューでは選択要素のHTMLを書き換えて保存済み文書を差し替え（revision が上がり自動リロード）。記録は `manualEdit: true` の通知専用コメントとして残り、エージェントには `wait-comments` で伝わるが**画面や未解決数には出ない**（差分は変更後の行のみ・読み取り専用モード中は不可）。
- **リポジトリファイルビューア**: サイドバー下部の「リポジトリのファイル」（デフォルト折りたたみ）からリポジトリのファイル（追跡ファイル＋未追跡ファイル。`.gitignore` 対象は除外）をツリーで辿り、差分に含まれないファイルも読み取り専用パネル（ピンスタック）として差分の横に並べて参照できる。差分の帯やツリーの「↗」（別タブで開く）は、このツリー付きの `/files` ページを**該当ファイルを開いた状態・左ツリーをそこまで展開した状態**で開くので、隣のファイルにそのまま辿れる（単独ページ `/file/<path>` も URL としては残っている）。配信はこの一覧に載っているファイルのみ（gitignore 済みの `.env` 等は出ない。差分生成と同じ境界）。ファイル内検索（`git grep`）だけは git の挙動どおり追跡ファイルのみが対象。
- **コメント当時のコードを保存**: 行・範囲コメントを投稿すると、その時点で画面に出ていた対象行と前後5行を `code` として保存する。修正が進んで差分が再生成され、コメントの行番号が本来の位置からずれていっても、コメントカードの「▶ コメント当時のコード」を開けば**指摘した当時のコードがそのまま**読める（既定は折りたたみ）。差分と同じ Shiki のハイライトが付く（コメント本文のコードフェンスと同じで、投稿時にサーバー側でトークン化して保存するため、ブラウザは Shiki を必要としない）。エージェントにも `wait-comments` で届くが、色情報は配達時に落とされ素のテキストだけが渡る。
- **コメントの Markdown 表示**: コメント本文もエージェントの返信も Markdown としてレンダリングされる（見出し・箇条書き・ネストしたリスト・番号付きリスト・チェックリスト・コードブロック・インラインコード・引用・水平線・リンク・強調・表）。レンダラは依存ライブラリなしの自前実装（`src/client/markdown.ts`）で、本文を先にエスケープしてから記法を組み立てるため、別途サニタイザを挟まずとも本文中の生HTMLが実行されることはない。
- **コメントへの画像添付**: どのコメントフォーム（行・全体・返信・HTMLレビュー）でも、スクリーンショット等をテキスト欄に**ペーストするだけで添付**できる（png/jpeg/gif/webp、1枚3MBまで・アップロード時に長辺1568pxへ自動縮小）。ペースト位置には `[画像: id]` 形式のマーカーが本文へ自動挿入され、本文中のどこの話をしている画像かが分かるようになる。投稿後のコメントカードでは、このマーカーの位置にその画像がインライン表示される。画像はブランチの `images/` にファイル保存され、コメントには id 参照だけが残る。`wait-comments` はエージェントに base64 ではなく**絶対ファイルパス（`imagePaths`）**で届けるため、エージェントのコンテキストにはパス文字列しか入らず、必要なコメントの画像だけを Read ツールで参照できる（トークン消費を最小化）。
...
- **push 依頼ボタン**: ヘッダ右側（「レビュー終了」の左隣）の「push を依頼」で、現在のブランチの push をエージェントに頼める。専用 API は無く、本文が `【push依頼】` で始まる全体コメント（`file: null`・`intent: "fix"`）として投稿されるだけなので、`wait-comments` で通常どおり届き、全体コメント欄に依頼の記録が残る。エージェントは未コミットの変更の扱いを確認したうえで push し、結果（ブランチ名・sha）を返信して resolve する。読み取り専用モード中は無効化される（このモード中の投稿は `intent: "question"` に強制されるため、依頼として成立しない）。文書レビュー（`publish-html`）のページには出ない。
...
基本の使い方は CLI を直接叩くのではなく、Claude Code のセッションで `/my-interactive-review` スキルに任せる。スキルがレビューHTMLの生成・サーバー起動・コメント待機・修正・resolve のループをすべて回すので、ユーザーはブラウザでコメントを書くだけでよい。使い方は次の3パターン。

### 1. AI コーディングした内容をレビューする

Claude Code に修正させたあと、同じセッションで:
...
先にエージェント自身にレビューさせたい場合は「まず自分でレビューして、指摘をブラウザに出して」と頼むと **AI レビューモード**になる。指摘が AI 名義のコメント（紫の「AI」バッジ）として画面に並び、返信（または「🔧 修正を依頼」ボタン）した指摘だけが修正され、放置した指摘はレビュー終了時に自動で見送りになる。

### 2. 実装プラン・テスト結果などの HTML レポートをレビューする

diff ではなく、エージェントが作った実装プラン・設計書・E2Eテスト結果などをレビューしたい場合:

```
この実装プランを my-interactive-review でブラウザレビューさせて
```

エージェントが対象を自己完結HTMLにして `publish-html` で登録し、`http://localhost:<実ポート>/doc/<id>` を案内してくる。ページはレンダリングされた状態のまま表示され、「要素を選択してコメント」ボタンで要素クリック、または文章のドラッグ選択でコメントできる。エージェントがプランを修正して再登録すると、ブラウザは自動リロードし、既存コメントは可能な範囲で元の位置に再配置される。

### 3. 他人の修正内容をレビューする（ブランチ指定）

同僚の MR / PR のブランチをローカルに checkout してレビューする使い方。コードを変更してはいけないので**読み取り専用モード**と組み合わせる:
...
```
このブランチの変更を main 基準で my-interactive-review でレビューしたい。読み取り専用で
```

エージェントが `main` との merge-base を base にレビューHTMLを生成する（ブランチ全体の変更が差分になる）。読み取り専用モードはブラウザの歯車メニューからも ON/OFF でき、ON の間エージェントはコードを一切変更せず、コメントへの調査結果・修正案の回答（`answered`）だけを返す。「まず AI に指摘を出させてから自分で見る」よう AI レビューモードと組み合わせるのも有効。
...
```bash
agent-review-kit generate                          # 初回: working tree vs HEAD の差分
...
| GET / PUT | `/api/viewed` | Viewed（確認済み）状態の取得 / 全置換 |
| POST | `/api/viewed/reconcile` | 現在の差分ハッシュと突合し、変わったファイルの Viewed を解除 |
| POST | `/api/edit` | 手動修正: 行範囲をファイルに直接適用し、差分を再生成 |
| POST | `/api/documents/:id/edit` | 手動修正（HTMLレビュー）: 保存済み文書の本文を差し替えて revision を上げる |
| POST | `/api/finish` | レビュー終了（AI指摘を一括見送り→サーバー停止） |
| GET | `/api/repo-files` | リポジトリファイルの一覧（追跡＋未追跡、ignore 対象は除外。サイドバーのリポジトリツリー用） |
| GET | `/api/file?path=...` | リポジトリファイル1件の内容（Shiki ハイライト付き、1MB上限・バイナリ判定あり） |
| GET | `/file/:path` | リポジトリファイル1件の読み取り専用ページ（画面の「↗」は代わりに `/files?file=<path>` を開く） |
| GET | `/files?file=...` | ファイルツリーのページ。`file` を付けるとそのファイルを開き、左ツリーをそこまで展開する |
...
`file` を送り `side` と行番号をすべて省略（または `null`）すると、そのファイル**全体**へのコメントになる（`side` と各行番号は `null`）。画面では差分の帯の「💬」から投稿し、帯の直下にスレッドが並ぶ。`side` を送る場合は従来どおり行アンカー4つ（`startLine`〜`endDiffLine`）が必須で、欠けていれば 400。
...
`POST /api/images` は**コメント添付画像のアップロードAPI**。リクエストボディに画像バイナリをそのまま送ると、マジックバイトで形式を判定（png/jpeg/gif/webp のみ、Content-Type ヘッダは信用しない）してブランチの `images/` に保存し、`{ "id": "img_xxx.png" }` を返す。3MB 超は `413`、画像でないデータは `400`、レビュー終了後は `409`。保存済み画像は `GET /api/images/:id` で配信される（id は immutable なのでキャッシュ可）。`POST /api/comments` は任意で `images`（アップロード済み id の配列、最大8枚）を受け取り、全コメント形（行・全体・返信・HTMLレビュー）でコメントに保存される。未アップロードの id や形状不正の id は `400`。

`POST /api/comments` は行・範囲コメントで任意の `code`（`{ "before": [...], "lines": [...], "after": [...] }` = コメント当時のコードのスナップショット）を受け取る。ブラウザが投稿時に画面の差分データから組み立てて送り、そのままコメントに保存される。全体コメント（`file` が null）では無視される。返信は親から `code` を継承する。壊れた形・上限超過（合計401行以上、または1行2001文字以上）のときは **400 にせず `code` フィールドだけを落として**コメント自体は保存する（スナップショットは補助情報で、これを理由にユーザーのコメントを失わせない）。

保存時にサーバーが `file` の拡張子から言語を判定して Shiki でトークン化し、`code.tokens` として書き込む（`before` + `lines` + `after` を1回でまとめてトークン化するので、テンプレートリテラルやブロックコメントが文脈行と対象行をまたいでも色が崩れない）。言語不明・ハイライト失敗時はフィールドごと省かれ、画面は素のテキスト表示にフォールバックする。
...
`POST /api/edit` は**手動修正**の適用API。`file` / `startLine`〜`endLine` / `startDiffLine`〜`endDiffLine` / `expectedText`（画面が表示していた範囲のテキスト） / `newText`（置き換え後のテキスト、空文字なら行削除）を送ると、working tree のファイルの該当行を書き換えて差分を再生成する。ディスク上の内容が `expectedText` と一致しない場合（最後の generate 以降にファイルが変わった等）は `409`（`stale`）で何も変更しない。適用時は `【手動修正】` で始まる `manualEdit: true` の user コメント（`status: open`）が修正後の行範囲に自動登録される。これは**通知専用**で、画面（スレッド・コメント一覧）と集計（未解決数・total）には一切出ないが、`wait-comments` の配達には含まれ、エージェントに「ファイルが手で変わった」ことが伝わる（本文に対応不要の旨を含むため、エージェントは確認して resolve するだけでよい）。

`POST /api/documents/:id/edit` はHTMLレビュー側の手動修正API。ブラウザが保存済み本文（`/doc/<id>/content`）を取得して detached DOM 上で選択要素の `outerHTML` を書き換え（ライブの iframe DOM はコメントマーク等が注入されているため直列化しない）、再直列化した**文書全体**を `html` として送る。`expectedRevision`（編集の元にした revision）が現在の revision と一致しない場合は `409`（`stale`）で何も変更しない。成功すると本文を差し替えて revision を +1 し（開いているページは revision 監視で自動リロード）、diff 側と同じく `manualEdit: true` の通知コメント（`documentId` 付き）を記録する。本文には「再 publish すると手動編集が上書きされる」旨の注意が入り、エージェントは以後この文書を更新する際に保存済みの現在の内容を基にできる。読み取り専用モード中は `403`。対象はプロジェクト内の通常ファイルのみ（プロジェクト外・`.agent-review/` 配下・symlink は 400）。読み取り専用モード中は `403`。
...
```bash
agent-review-kit wait-comments --timeout 0     # 無期限に待つ（デフォルト）
...
コード スナップショット付きのコメントは、`code.tokens`（Shiki のハイライト）を**落として**届く。素のテキスト（`before` / `lines` / `after`）はそのまま入る。色情報はエージェントの役に立たず、コードそのものの数倍のコンテキストを食うためで、`images` と同じく **stdout 用のコピーを作るときに外しているだけ**（`comments.json` は書き換えない）。

画像付きコメントは、保存形式の `images`（id 配列）が**絶対ファイルパスの `imagePaths` に差し替えられて**届く。base64 を stdout に流さないため、エージェントのコンテキストにはパスしか入らない。画像付きのバッチには参照方法を説明する `imagesNote` が同乗するので、消費側に事前知識は不要:

```json
{ "status": "received", "imagesNote": "imagePaths はユーザーがコメントに添付した画像ファイルの絶対パス。…", "comments": [ { "id": "comment_xxx", "body": "この画面崩れを直して", "imagePaths": ["/path/to/repo/.agent-review/branches/main/images/img_xxx.png"] } ] }
```
...
agent-review-kit add-comment --file src/foo.ts --body "..."  # 行なし = ファイル全体への指摘
...
「🔧 修正を依頼」ボタン。このボタンは未解決のコメントカードすべて（通常コメント・返信を
含む）に出る）すると、その返信が通常のコメントとして届く。未返信の指摘は
...
- 「ドキュメント全体にコメント」も可能
- 要素選択・テキスト選択で開くフォームの「✏️ 手動修正」ボタンで、選択要素（テキスト選択の場合はそれを含む要素）のHTMLをその場で書き換えて保存済み文書に直接適用できる（`POST /api/documents/:id/edit`）。空にして保存すると要素を削除。ドキュメント全体へのコメントフォームには出ない
  ...
  "comments": [
    {
      ...
      "code": {
        "before": ["function load(id) {", "  const row = find(id);"],
...
- **ファイル全体コメント**は `file` だけを持ち、`side` と各行番号が `null`。行アンカーが無いので差分表の中には入らず、そのファイルの帯の直下に表示される（サイドバーの表示は `パス 全体`）。
