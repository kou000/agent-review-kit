...
- コメントごとに別プロセスの Claude Code を起動しない。このセッションがループの主体。
- 一度に複数コメントを受け取った場合は、同じファイルへの変更が競合しないよう、ファイル単位・行番号順に順序立てて処理する。
- 修正前に必ず現在の git diff を確認し、修正後に必ずテストまたは型チェックを実行する。
- `wait-comments` は長時間ブロックするコマンドなので、バックグラウンド実行にして完了通知を待つ形にしてよい。その間も他の作業はできるが、コメント対応中の修正と競合させないこと。
