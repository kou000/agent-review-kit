...
- `intent: "fix"` — 本文で判断する（修正指摘なら修正、実質的に質問なら回答）。
- `intent` が無いコメント（`add-comment` で投稿した AI 指摘、古い `comments.json`）も本文で判断する。
- 本文が `【push依頼】` で始まる全体コメント（`file: null`、ブラウザ右上の「push を依頼」ボタンから投稿される） — 現在のブランチを push してほしいという依頼。コード修正ではないのでサブエージェントに委譲せず、メインセッションが次の順で対応する:
  1. `git status --short` で未コミットの変更を確認する。未コミットの変更がある場合は、勝手にコミットせず「コミットしてから push するか、コミット済み分だけ push するか」を `--status answered` の返信でユーザーに確認して止まる（ユーザーの返信は同じスレッドの `parentId` 付きコメントとして届く）。
  2. push する（`git push -u origin <ブランチ>`。upstream 設定済みなら `git push`）。force push はユーザーが明示した場合以外は行わない。
  3. 成功したら `resolve-comment <id> --status resolved --message "push しました: <ブランチ名> @ <短い sha>"` で返信して resolve する。失敗したら出力を添えて `--status answered` で返す（resolve しない）。
  ブラウザは `readOnlyMode: true` の間このボタンを無効化する（サーバーがこのモード中の投稿を `intent: question` に強制するため）。したがって push 依頼が届くのは読み取り専用 OFF のときだけで、届いた依頼の `intent` は常に `fix` になる。
...
5. 受け取った各コメントを読む。`intent` / `file` / `side` / `startLine`〜`endLine` / `body` を確認する。
   ...
   - `file: null` のコメントは特定の行ではなくレビュー全体への指摘・質問（`side` / 各行番号も null）。本文が `【push依頼】` で始まるものは push 依頼（「コメント種別（intent）の扱い」の項を参照）。
   - 行範囲コメント（startLine ≠ endLine）の場合は、範囲全体のコードを読んで指摘の意図を解釈する。1行だけ見て対応しない。
   - `code`（`before` / `lines` / `after`）は**そのコメントが書かれた時点のコード**のスナップショット。修正が進むと `startLine`〜`endLine` は現在のファイルの別の場所を指すようになるので、**行番号と `code` が食い違うときは `code` の方が指摘対象**である。`code.lines` の内容を現在のファイルから検索して本当の対象行を特定してから対応すること（`code` が無いコメントは行番号で辿る）。
...
2. レビュー対象の diff を読み、観点（バグ・セキュリティ・性能・可読性・規約）ごとにレビューする。指摘は1件ずつコメントとして投稿する。**本文は Markdown で書く**（「コメント本文の書き方（Markdown）」参照）:
   ...
   # ファイル全体（どの行にも属さない指摘: 構成・責務・そもそも触る必要があるか）
   agent-review-kit add-comment --file src/foo.ts --body "指摘内容"
   # レビュー全体への所感
...
4. **自分が投稿した指摘（AI コメント）はこの時点では処理しない。** wait-comments にも流れてこない（配達されるのはユーザー名義の open コメントだけ）。ユーザーが AI 指摘に**返信**したら、その返信が通常のコメントとして届くので、スレッド全体（親=自分の指摘）を読んで修正または回答する。修正の流れは手順7と同じ。返信の `intent` が `question` なら、親が自分の修正指摘であっても**修正せず回答だけ**する（画面の「🔧 修正を依頼」ボタンからの返信は常に `intent: "fix"`。このボタンは AI 指摘だけでなく未解決のコメントカードすべて（通常コメント・返信を含む）に出るので、通常コメントのスレッドにも、スレッド途中からも定型の修正依頼返信が届きうる）。
...
- 直し方が指摘から一意に決まる機械的な修正（typo・命名・定型の書き換えなど）は、サブエージェントに**セッションより軽いモデルを相対指定**する。どう直すかの判断が要る修正は、セッションと同等のモデルを使う（特定のモデル名をスキルにハードコードしない）。
...
- **1 エージェントに複数コメントの修正を任せるときも、スナップショットはコメント単位を崩さない**: メイン working tree で連続修正させて最後に 1 つの合算スナップショットにすると、各コメントの「修正差分」リンクが全修正混在になり読めない。複数件を任せる場合は worktree で**修正ごとにコミット**させ、`snapshot create --comment <id> --commit <sha>` をコメントごとに作ってから resolve する。メイン tree 直接編集で進めてしまった場合は、修正間で main 側から snapshot begin/create を挟む。
