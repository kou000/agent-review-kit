...
5. 受け取った各コメントを読む。`intent` / `file` / `side` / `startLine`〜`endLine` / `body` を確認する。
   ...
   - `code`（`before` / `lines` / `after`）は**そのコメントが書かれた時点のコード**のスナップショット。修正が進むと `startLine`〜`endLine` は現在のファイルの別の場所を指すようになるので、**行番号と `code` が食い違うときは `code` の方が指摘対象**である。`code.lines` の内容を現在のファイルから検索して本当の対象行を特定してから対応すること（`code` が無いコメントは従来どおり行番号で辿る）。
